...
import { 
  ...
  Loader2, AlertCircle, ArrowUpRight, ArrowDownRight, Activity, Calendar, LayoutTemplate, Target, PieChart
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  if (activeTab === 'telegram') {
    ...
    // 🌟 整合後的 Landing Page 核心指標
    cardsConfig = [
      { key: 'visitors', title: 'Landing Page 訪客', icon: LayoutTemplate, color: 'bg-rose-500' },
      { key: 'leads', title: '名單獲取 (Leads)', icon: Target, color: 'bg-amber-500' },
      { key: 'conversionRate', title: '轉換率 (CVR)', icon: PieChart, color: 'bg-indigo-500' },
      { key: 'avgCost', title: '獲客成本 (Organic)', icon: TrendingUp, color: 'bg-emerald-500' }
    ];
  } else {
    // Overview
    cardsConfig = [
      ...
      { key: 'leads', title: '總名單數', icon: Users, color: 'bg-emerald-600' }
    ];
  }
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
      <aside className="w-64 bg-white border-r border-slate-200 fixed h-full z-20 hidden md:flex flex-col">
        ...
        <nav className="p-4 space-y-1">
          ...
          {/* 🌟 整合的 Landing Page 選項 */}
          <button onClick={() => setActiveTab('website')} className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-colors ${activeTab === 'website' ? 'bg-rose-50 text-rose-600' : 'text-slate-600 hover:bg-slate-50'}`}><LayoutTemplate size={18} /> Landing Page (官網)</button>
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 overflow-y-auto">
        <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold capitalize">{activeTab === 'website' ? 'Landing Page Analytics' : activeTab + ' Dashboard'}</h1>
              ...
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="colorMain" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={activeTab === 'website' ? "#e11d48" : isTelegram ? "#0088cc" : "#6366f1"} stopOpacity={0.2}/>
                    <stop offset="95%" stopColor={activeTab === 'website' ? "#e11d48" : isTelegram ? "#0088cc" : "#6366f1"} stopOpacity={0}/>
                  </linearGradient>
                </defs>
                ...
                <Tooltip />
                <Area type="monotone" dataKey={isTelegram ? "msgSent" : "value"} stroke={activeTab === 'website' ? "#e11d48" : isTelegram ? "#0088cc" : "#6366f1"} fillOpacity={1} fill="url(#colorMain)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
